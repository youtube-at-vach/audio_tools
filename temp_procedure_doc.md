...
#### 8.2. Wow and Flutter Analyzer 開発からの学び (Learnings from Wow and Flutter Analyzer Development):

-   **入力データの検証（早期かつ頻繁に）(Input Data Validation - Early and Often)**:
    -   オーディオ入力データは、読み込み直後に検証します（例：`None`でないか、空でないか、十分な長さがあるか、サンプルレートが有効か）。処理パイプラインの下流にある関数も、それぞれの入力配列を検証すべきです（例：フィルタリングやFFTのような複雑な操作を試みる前に、空でないか、全てNaNでないか、不適切に短くないかを確認する）。
    -   無効または不十分な入力のために意味のある出力を生成できない信号処理関数は、明確に定義された値（例：空のNumPy配列 `np.array([])`）を返し、コンソールに警告を発するべきです。これにより、エラーの伝播を防ぎ、デバッグを支援します。

-   **堅牢な数値計算（NaN/Inf安全性）(Robust Numerical Calculations - NaN/Inf Safety)**:
    -   数値計算（例：メトリクス計算）を実行する際は、必要に応じてNaN対応のNumPy関数（例：`np.nanmean`, `np.nanmax`, `np.nanmin`）を使用します。
    -   計算結果が後続の計算（特に除算）で使用される前や他の関数に渡される前に、`np.nan` または `np.inf` が含まれていないか明示的に確認します（`np.isfinite()` を使用）。非有限値が検出された場合は、適切なデフォルト値（例：`0.0`）に置き換え、警告をログに記録すべきです。
    -   ゼロまたはゼロに近い参照値に対するパーセンテージ計算など、ゼロ除算の可能性には特に注意を払います。チェックと安全なフォールバックを実装します。

-   **防御的なプロット実践 (Defensive Plotting Practices)**:
    -   データプロットを試みる前（例：`matplotlib.pyplot.plot(x, y)`を使用）、`x` と `y` の両データセットが有効で互換性があることを確認します。特に、`x` がデータを含む場合、`y` もデータを含み、互換性のある形状を持つことを検証します。空でない配列に対して空の配列をプロットすると `ValueError` が発生する可能性があります。
    -   個々のプロットコンポーネントのデータが欠落しているか無効な場合は、条件付きでプロットをスキップし、コンソールメッセージを介してユーザーに通知します。
    -   プロットの軸の範囲を計算する際（例：`ax.set_ylim()`経由）、計算された範囲の値が有限であり、有効な範囲（最小値 < 最大値）を形成することを確認します。そうでない場合は、matplotlibに自動スケーリングさせるか、汎用的なデフォルト範囲を設定するか、範囲設定をスキップし、常に警告をログに記録します。これにより、範囲計算用のデータ自体に問題がある場合のクラッシュを防ぎます。

-   **テストとフィードバックによる反復的な改善 (Iterative Refinement through Testing and Feedback)**:
    -   実際の使用や特定のテストケースから報告されるランタイムエラーは、データ処理におけるエッジケースを発見するために非常に貴重です。これらの問題を特定し、エラーハンドリングを強化し、これらのシナリオをカバーする特定のユニットテストを追加するという反復的なプロセスを受け入れます。
    -   ユニットテストは、「ハッピーパス」シナリオだけでなく、空の入力、NaN/Inf値、非常に短い信号、その他予見可能な問題のあるデータでの動作を明示的にテストし、堅牢性を確保すべきです。
