...
def process_stereo_diffuse_side_3d_block(
    ...
    *,
    ...
    Parameters reuse (since GUI already has them):
    - alpha: diffusion depth in [0,1]
    - beta : width amount in [0,1]
    - tau_seconds: controls base delay time (clamped to a sane range)
    """
    if state is None:
    ...
    # Map tau to a reasonable base delay range (ms).
    ...
    # (Re)allocate ring buffer if needed.
    ...
    # High-pass coefficient for side.
    fc = 150.0
    a = float(np.exp(-2.0 * np.pi * float(fc) / float(sample_rate)))

    # Tap gains: small to avoid obvious reverb/comb.
    g1 = 0.35 * depth
    g2 = 0.22 * depth
    g3 = 0.14 * depth
    dry = 1.0 - 0.55 * depth
    ...
    for i in range(n):
        ...
        # If effectively silent, keep output silent and slowly decay ring state.
